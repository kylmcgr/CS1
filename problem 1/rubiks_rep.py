...
class RubiksRep:
    ...
    def rotate_f(self):
        '''
        Move the f slice one-quarter turn clockwise.
        '''
        assert self.size > 3
        r = self.get_col('R', 1)
        l = self.get_col('L', self.size - 2)
        u = self.get_row('U', self.size - 2)
        d = self.get_row('D', 1)
        self.set_row('U', self.size - 2, l[::-1])
        self.set_row('D', 1, r[::-1])
        self.set_col('R', 1, u)
        self.set_col('L', self.size - 2, d)

    def rotate_s(self):
        '''
        Move the s slice one-quarter turn clockwise.
        '''
        assert self.size % 2 != 0
        r = self.get_col('R', int((self.size - 1) / 2))
        l = self.get_col('L', int((self.size - 1) / 2))
        u = self.get_row('U', int((self.size - 1) / 2))
        d = self.get_row('D', int((self.size - 1) / 2))
        self.set_row('U', int((self.size - 1) / 2), l[::-1])
        self.set_row('D', int((self.size - 1) / 2), r[::-1])
        self.set_col('R', int((self.size - 1) / 2), u)
        self.set_col('L', int((self.size - 1) / 2), d)
